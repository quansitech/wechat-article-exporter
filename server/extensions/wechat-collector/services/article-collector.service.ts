  ...
  private async fetchArticles(account: CollectionAccount, maxArticles: number): Promise<any[]> {
    ...
    while (hasMore && allArticles.length < maxArticles) {
      try {
        // 重用 API (需要适配参数)
        const accountInfo = {
          fakeid: account.id, // 使用 id 作为 fakeid
          nickname: account.nickname,
        } as any;
        
        const [articles, completed] = await getArticleList(accountInfo, begin);
        ...
        if (completed || allArticles.length >= maxArticles) hasMore = false;
